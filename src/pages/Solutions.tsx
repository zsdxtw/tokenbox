...
const categoryColors: Record<string, { bg: string; text: string; border: string; dot: string }> = {
  基础设施建设: { bg: "bg-brand-50", text: "text-brand-600", border: "border-brand-200", dot: "bg-brand-500" },
  ...
  运维管理: { bg: "bg-brand-50", text: "text-brand-600", border: "border-brand-200", dot: "bg-brand-500" },
};
...
export default function Solutions() {
  ...
  return (
    <div className="bg-white min-h-screen">
          ...
          <div className="max-w-3xl">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <div className="px-4 py-2 bg-white border border-ink-200 rounded-lg text-sm shadow-soft">
                <span className="text-ink-500">自营内容：</span>
                <span className="text-brand-600">算力巢专家团队设计</span>
              </div>
              <div className="px-4 py-2 bg-white border border-ink-200 rounded-lg text-sm shadow-soft">
                <span className="text-ink-500">已落地：</span>
                <span className="text-amber-600">{solutionCases.length} 个案例</span>
              </div>
            </div>
          </div>
        </div>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive ? "border-brand-400 shadow-card" : "border-ink-200 hover:border-brand-300"
                )}
              >
                ...
                <h3 className="text-xl font-bold text-ink-900 mb-1">{scenario.name}</h3>
      ...
      {/* Scenario detail */}
      <section id="scenario-detail" className="bg-ink-50/50 border-y border-ink-100">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="lg:col-span-2 space-y-6">
                    <div className="p-6 bg-white rounded-xl border border-ink-200 shadow-soft">
                  ...
                  <div className="space-y-4">
                    ...
                    <button className="w-full py-3 bg-gradient-to-r from-brand-500 to-brand-600 hover:shadow-glow text-white font-medium rounded-lg transition-all flex items-center justify-center gap-2">
                      咨询方案
          ...
          {solutionCases.map((caseItem) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="text-lg font-bold text-ink-900 leading-snug mb-2">
                  {caseItem.title}
                </h3>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
                  {caseItem.results.map((r) => (
                    <div key={r.label} className="p-2.5 bg-ink-50 rounded-lg text-center">
                      <div className="font-mono text-sm font-bold text-gradient-brand">{r.value}</div>
                ...
                {isExpanded && (
                  <div className="mt-4 p-4 bg-brand-50/50 rounded-lg">
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2">
                      {caseItem.results.map((r) => (
                        <div key={r.label} className="flex items-center justify-between text-xs">
                          ...
                          <span className="font-mono font-semibold text-brand-700">{r.value}</span>
                        </div>
                      ))}
                    </div>
                  </div>
