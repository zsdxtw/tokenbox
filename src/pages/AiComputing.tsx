...
export default function AiComputing() {
  ...
  useEffect(() => {
    setSelectedCategory('ai-computing');
  ...
  return (
    ...
    >
      ...
      <nav className="flex items-center gap-1.5 text-sm text-nest-muted mb-6">
        ...
        <span className="text-nest-text">AI算力馆</span>
      </nav>

      {/* Page Title */}
      <div className="flex items-center gap-3 mb-6">
        <div className="w-10 h-10 rounded-lg bg-nest-blue/10 flex items-center justify-center">
          <Cpu className="w-5 h-5 text-nest-blue" />
        </div>
        <h1 className="font-display text-2xl font-bold text-nest-text">AI算力馆</h1>
      </div>

      {/* Layout */}
      <div className="flex gap-6">
        <CategorySidebar category="ai-computing" />
        <div className="flex-1 space-y-4">
          <ProductFilter category="ai-computing" />
